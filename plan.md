...
- 2026-05-05 · Step 4 · Wrote `scripts/fetch_scholar.py` (uses `scholarly` to pull pubs by ID, soft-fails on rate-limit so cron never breaks the site, sorts year-desc/title-asc) and `.github/workflows/scholar-sync.yml` (daily 07:00 UTC + manual dispatch, commits diff via github-actions bot).
